...
const routeLabels = { '/': 'الرئيسية', '/recitations': 'التلاوات', '/quran': 'مصحف', '/hadith': 'الأحاديث النبوية', '/prophets': 'قصص الأنبياء عليهم السلام', '/adhkar': 'الأذكار والأدعية', '/seerah': 'السيرة النبوية', '/search': 'البحث الشامل', '/sources': 'المصادر والتخريج', '/sunnah': 'سنن النبي' }
const sheikhsData = [
...
export default function App() {
  ...
  return <div dir="rtl" className="site-shell">
    <header className="topbar"><div className="topbar-inner"><button className="brand" onClick={() => go('/')} aria-label="العودة إلى الرئيسية"><img className="brand-logo" src="/tilawat-haramain-wordmark.jpg" alt="تلاوات الحرمين" draggable="false" /></button><div className="header-actions"><div className="theme-menu" ref={themeMenuRef}><button className="theme-toggle" onClick={() => { setIsDropdownOpen(false); setIsThemeOpen((value) => !value) }} aria-haspopup="listbox" aria-expanded={isThemeOpen} title="اختيار المظهر">المظهر<span className="theme-chevron">⌄</span></button>{isThemeOpen && <div className="theme-options" role="listbox" aria-label="خيارات المظهر"><button className={theme === 'dark' ? 'selected' : ''} onClick={() => chooseTheme('dark')} role="option" aria-selected={theme === 'dark'}>داكن</button><button className={theme === 'light' ? 'selected' : ''} onClick={() => chooseTheme('light')} role="option" aria-selected={theme === 'light'}>فاتح</button><button className={theme === 'system' ? 'selected' : ''} onClick={() => chooseTheme('system')} role="option" aria-selected={theme === 'system'}>مع النظام</button></div>}</div><button className="mobile-menu" onClick={() => { setIsThemeOpen(false); setIsDropdownOpen((v) => !v) }} aria-label="فتح القائمة">☰</button></div></div>
      <nav className={`main-nav ${isDropdownOpen ? 'nav-open' : ''}`} aria-label="التنقل الرئيسي"><button className={route === '/' ? 'active' : ''} onClick={() => go('/')}>الرئيسية</button><button className={route === '/recitations' ? 'active' : ''} onClick={() => go('/recitations')}>التلاوات</button><button className={route === '/quran' ? 'active' : ''} onClick={() => go('/quran')}>مصحف</button><button className={route === '/hadith' ? 'active' : ''} onClick={() => go('/hadith')}>الأحاديث النبوية</button><button className={route === '/prophets' ? 'active' : ''} onClick={() => go('/prophets')}>قصص الأنبياء</button><button className={route === '/adhkar' ? 'active' : ''} onClick={() => go('/adhkar')}>الأذكار والأدعية</button><button className={route === '/sunnah' ? 'active' : ''} onClick={() => go('/sunnah')}>سنن النبي</button></nav></header>
    <main className="main-content">{route === '/' && <Home go={go} />}{route === '/recitations' && <Recitations current={currentRecitation} selected={selectedSheikh} setSelected={setSelectedSheikh} open={isSheikhOpen} setOpen={setIsSheikhOpen} menuRef={sheikhMenuRef} audioRef={audioRef} onTime={saveAudio} />}{route === '/sunnah' && <SunnahPage />}{['/quran', '/hadith', '/prophets', '/adhkar', '/seerah', '/search', '/sources'].includes(route) && <ComingSoon route={route} go={go} />}</main>
    <img className="national-strip" src="/national-day-strip.png" alt="شريط بمناسبة اليوم الوطني السعودي" draggable="false" />
    <footer className="footer"><div><strong>tilawat haramain</strong><p>محتوى إسلامي، تلاوات خاشعة وأحاديث.</p></div><div className="footer-links"><button onClick={() => go('/sources')}>المصادر والتخريج</button><button onClick={() => go('/search')}>البحث الشامل</button></div><div className="footer-bottom"><span>© 2026 tilawat haramain</span></div></footer>
  </div>
}
function Home({ go }) { return <section className="home-page"><div className="hero-card"><span className="eyebrow">من الحرمين الشريفين</span><h1>صوتٌ يفتح <em>أبواب السكينة</em></h1><p>محتوى إسلامي، تلاوات خاشعة وأحاديث</p><div className="hero-actions"><button className="primary-btn" onClick={() => go('/recitations')}>استمع إلى التلاوات</button><button className="secondary-btn" onClick={() => go('/quran')}>افتح المصحف</button></div></div></section> }
